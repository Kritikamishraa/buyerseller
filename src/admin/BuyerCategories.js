...
const BuyerCategories = () => {
  ...
  useEffect(() => {
    dispatch(adminGetAllBuyerCategories());
  }, [dispatch]);

  const handleDelete = (id) => {
    Modal.confirm({
      title: "Are you sure you want to delete this buyer category?",
      okText: "Yes, Delete",
      okType: "danger",
      cancelText: "Cancel",
      onOk: async () => {
        try {
          ...
          AntMessage.success("Buyer category deleted successfully");
          dispatch(adminGetAllBuyerCategories());
        } catch (err) {
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    const title = "Buyer Categories Report";
    ...
    doc.save(`Buyer_Categories_${new Date().toISOString().slice(0, 10)}.pdf`);
  };
  ...
  return (
    <Box>
      ...
      >
        ...
        <Typography sx={{ color: "primary.main" }}>Buyer Categories</Typography>
      </Breadcrumbs>
